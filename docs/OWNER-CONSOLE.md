...
Use **Add manual lead** for referrals, walk-ins and direct telephone enquiries. Enter the customer name,
Indian mobile number, city, preferred Hindi/English/Marathi language and an optional source reference.
Choose to call as soon as allowed, schedule a specific time, or save without calling. Scheduled browser
time is converted to an absolute timestamp; the voice worker still enforces Monday-Saturday 10:00-19:00
IST. Existing phone numbers are returned as duplicates and never create a second lead or call.
